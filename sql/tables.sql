...
DROP TABLE IF EXISTS [dbo].[PRODUCT_INVENTORY_RECORDS]
DROP TABLE IF EXISTS [dbo].[PRODUCT_INVENTORY_RECORD_TYPES]

DROP TABLE IF EXISTS [dbo].[BATCH_STATUS_RECORDS]
DROP TABLE IF EXISTS [dbo].[BATCH_STATUSES]
DROP TABLE IF EXISTS [dbo].[BATCH]
DROP TABLE IF EXISTS [dbo].[BATCH_DELIVERY_TYPES]
DROP TABLE IF EXISTS [dbo].[BATCH_SOURCE_TYPES]
...
DROP TABLE IF EXISTS [dbo].[USERS]
    DROP INDEX IF EXISTS [dbo].[USERS].[USERS_SICIL_UNIQUE]
    DROP INDEX IF EXISTS [dbo].[USERS].[USERS_MAIL_UNIQUE]
...
CREATE UNIQUE INDEX USERS_SICIL_UNIQUE ON
    [dbo].[USERS] ([sicil])
    WHERE [is_active] = 1;
GO
...
CREATE TABLE [dbo].[BATCH_DELIVERY_TYPES](
	[id] INT IDENTITY(1, 1) PRIMARY KEY,
	[name] NVARCHAR (50) NOT NULL,
)

CREATE TABLE [dbo].[BATCH_SOURCE_TYPES](
	[id] INT IDENTITY(1, 1) PRIMARY KEY,
	[name] NVARCHAR (50) NOT NULL,
)
...
CREATE TABLE [dbo].[ICMAL](
	[id] INT IDENTITY(1, 1) PRIMARY KEY,
	[bank_id] INT NOT NULL,
        CONSTRAINT FK_ICMAL_BANKS FOREIGN KEY ([bank_id]) 
        REFERENCES [dbo].[BANKS]([id]) ,
	[description] NVARCHAR (500) NULL,
...
CREATE TABLE [dbo].[BATCH](
    ...
    [icmal_id] INT NOT NULL,
        CONSTRAINT FK_BATCH_ICMAL FOREIGN KEY ([icmal_id]) 
        REFERENCES [dbo].[ICMAL]([id]) ,
    [product_id] INT NOT NULL,
        CONSTRAINT FK_BATCH_PRODUCTS FOREIGN KEY ([product_id]) 
        REFERENCES [dbo].[PRODUCTS]([id]) ,
    [batch_delivery_type_id] INT NOT NULL,
        CONSTRAINT FK_BATCH_BATCH_DELIVERY_TYPES FOREIGN KEY ([batch_delivery_type_id]) 
        REFERENCES [dbo].[BATCH_DELIVERY_TYPES]([id]) ,
    [batch_source_type_id] INT NOT NULL,
        CONSTRAINT FK_BATCH_BATCH_SOURCE_TYPES FOREIGN KEY ([batch_source_type_id]) 
        REFERENCES [dbo].[BATCH_SOURCE_TYPES]([id]) ,
    ...
    [created_by] NVARCHAR(50) NULL,
        ...
        REFERENCES [dbo].[USERS]([username]) ,
    [edited_by] NVARCHAR(50) NULL,
        CONSTRAINT FK_BATCH_EDITED_BY_USERS FOREIGN KEY ([edited_by]) 
        REFERENCES [dbo].[USERS]([username]) ,
)

CREATE TABLE [dbo].[BATCH_STATUSES](
	[id] INT IDENTITY(1, 1) PRIMARY KEY,
	[name] NVARCHAR (50) NOT NULL,
)

CREATE TABLE [dbo].[BATCH_STATUS_RECORDS](
	[id] INT IDENTITY(1, 1) PRIMARY KEY,
    [batch_id] INT NOT NULL,
        CONSTRAINT FK_BATCH_STATUS_RECORDS_BATCH FOREIGN KEY ([batch_id]) 
        REFERENCES [dbo].[BATCH]([id]) ,
    [batch_status_id] INT NOT NULL,
        CONSTRAINT FK_BATCH_STATUS_RECORDS_BATCH_STATUSES FOREIGN KEY ([batch_status_id]) 
        REFERENCES [dbo].[BATCH_STATUSES]([id]) ,
	[created_at] DATETIME NOT NULL,
)
...
CREATE TABLE [dbo].[PRODUCT_INVENTORY_RECORD_TYPES](
	[id] INT IDENTITY(1, 1) PRIMARY KEY,
	[name] NVARCHAR (50) NOT NULL,
)

CREATE TABLE [dbo].[PRODUCT_INVENTORY_RECORDS](
	[id] INT IDENTITY(1, 1) PRIMARY KEY,
    [product_id] INT NOT NULL,
        CONSTRAINT FK_PRODUCT_INVENTORY_RECORDS_PRODUCTS FOREIGN KEY ([product_id]) 
        REFERENCES [dbo].[PRODUCT_TYPES]([id]) ,
    [product_inventory_record_type_id] INT NOT NULL,
        CONSTRAINT FK_PRODUCT_INVENTORY_RECORDS_PRODUCT_INVENTORY_RECORD_TYPES FOREIGN KEY ([product_inventory_record_type_id]) 
        REFERENCES [dbo].[PRODUCT_INVENTORY_RECORD_TYPES]([id]) ,
    [quantity] INT NOT NULL,
    [batch_id] INT NULL,
        CONSTRAINT FK_PRODUCT_INVENTORY_RECORDS_BATCH FOREIGN KEY ([batch_id]) 
        REFERENCES [dbo].[BATCH]([id]) ,
    [is_main_safe] BIT NOT NULL,
	[created_at] DATETIME NOT NULL,
    [created_by] NVARCHAR(50) NULL,
        CONSTRAINT FK_PRODUCT_INVENTORY_RECORDS_CREATED_BY_USERS FOREIGN KEY ([created_by]) 
        REFERENCES [dbo].[USERS]([username])
)